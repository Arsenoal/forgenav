...
import dev.forgenav.compose.nav.NavTransitions
    ...
    val parser = remember {
        DeepLinkParser(graph)
            .register(
                ...
                // Rebuild Home → Detail so back returns to the list (Nav3-style stack deep link).
    ...
    MaterialTheme {
        Scaffold(
            ...
            modifier = Modifier.onPreviewKeyEvent { event ->
                // Desktop Escape / Ctrl+[ ≈ system back
                if (event.type == KeyEventType.KeyUp &&
                    (event.key == Key.Escape || (event.isCtrlPressed && event.key == Key.LeftBracket)) &&
                    navigator.canPop
                ) {
