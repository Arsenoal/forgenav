...
class TaskViewModel(
    private val loop: LocalSyncForgeLoop = ForgeNavSync.localLoop(
        scope = kotlinx.coroutines.CoroutineScope(SupervisorJob() + Dispatchers.Default),
    ),
) : MviViewModel<TaskListState, TaskIntent, TaskEffect>(
    initialState = TaskListState(),
    parentContext = Dispatchers.Main,
    syncFacade = null, // we bind manually to the loop's facade streams
) {
    init {
    ...
    override suspend fun handleSideEffect(
        ...
        when (intent) {
            ...
            TaskIntent.GoOffline -> {
                ...
                // Force status refresh by touching outbox observation via a no-op sync attempt
