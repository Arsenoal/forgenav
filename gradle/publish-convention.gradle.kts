...
/** Library modules published to Maven Central. */
...
fun Project.javadocJarTask(): org.gradle.api.tasks.TaskProvider<Jar> {
    val existing = tasks.findByName("emptyJavadocJar")
    if (existing != null) {
        @Suppress("UNCHECKED_CAST")
        return tasks.named("emptyJavadocJar", Jar::class.java)
    }
    return tasks.register<Jar>("emptyJavadocJar") {
...
gradle.projectsEvaluated {
    subprojects
        ...
        .forEach { project ->
            ...
            val licenseName = project.providers.gradleProperty("forgenav.license.name")
            val licenseUrl = project.providers.gradleProperty("forgenav.license.url")
            val pomUrl = project.providers.gradleProperty("forgenav.pom.url")
            val scmConnection = project.providers.gradleProperty("forgenav.pom.scm.connection")
            val scmDeveloperConnection =
                project.providers.gradleProperty("forgenav.pom.scm.developerConnection")
            val pomInceptionYear = project.providers.gradleProperty("forgenav.pom.inceptionYear")
            val descriptionProperty = "forgenav.pom.description.${project.name}"
            val moduleDescription = project.findProperty(descriptionProperty)?.toString()
                ?: "ForgeNav — offline-first navigation and MVI for Kotlin Multiplatform"

            val javadocJar = project.javadocJarTask()

            project.extensions.configure<PublishingExtension>("publishing") {
                // KMP already creates publications; ensure JVM/android-only modules get one.
                ...
                publications.withType<MavenPublication>().configureEach {
                    ...
                    pom {
                        ...
                        developers {
                            developer {
                                name.set("ForgeNav Contributors")
                            }
                        }
